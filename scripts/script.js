...
function onYouTubeApiLoad() {
    gapi.client.setApiKey(''); //Use the youtube data api key
    ...
    $.ajax({
        ...
        success: function(response) {
            try {
                ...
                var popBody = "<img class='img-responsive center-block' src=" + event.data.value['snippet']['thumbnails']['medium']['url'] + "><div id='filesize'>Size: " + filesize + "MB</div><div id='bitrate'>Bitrate: " + bitrate + "kbps</div>";
                ...
                window.location.href = youtubeInMp3;
            }
        },
        error: function() {
            $('#download' + event.data.count).attr('disabled', false);
...
//enbleing enter to start the search 
function handle(e) {
    if (e.keyCode === 13) {
        search();
    }
